...
* `unactivatedAccountExpiry` in seconds [`undefined`]: if set to a number of seconds, the account will completely be removed from the system (log date will be retained) if no activation has been performed since the given number of seconds.
